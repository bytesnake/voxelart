...
void cubiverse::GUI::changeMenu(string p_menu) {
	//if(menu != "")	{
		if((p_menu == "Singleplayer") || (p_menu == "Multiplayer")) {
			menu=p_menu;
...
void cubiverse::GUI::drawMenu() {
	...
	/*glTranslatef(width/3, height-100.0f, 0.0f);
	std::string entry[] = {"Singleplayer", "Multiplayer", "Voxel", "Options", "Exit" };
	for(int i=0; i < sizeof(entry) / sizeof(entry[0]); i++) {
		glTranslatef(0.0f,-70.0f,0.0f);
		drawButton(entry[i], 200, 50);
	}*/
}
...
cubiverse::GUI::GUI(int width, int height) {
	std_font=new Bsb_Font("Data/test.png","Data/test.bfd");
